...
/**
 * SV-AUD-012: pin mcp-remote to an audited exact version. The launcher must not
 * resolve "latest" at runtime (mutable, unverified code running with the client
 * key). This is kept in sync with the mcp-remote entry in this package's
 * package.json so `npm ci` installs the same reviewed version.
 */
const MCP_REMOTE_VERSION = "0.1.38";
const MCP_REMOTE_SPEC = `mcp-remote@${MCP_REMOTE_VERSION}`;

/**
 * SV-AUD-007: write the MCP client key to a 0600 credential file and a launcher
 * that reads it at runtime, returning a {command, args} the tool config points
 * at. The returned args contain no secret material — only the launcher path,
 * which reads the key from the credential file when invoked.
 *
 * On Windows there is no POSIX file mode; the credential file is still written
 * but the launcher is a small Node script the config invokes directly.
 */
function writeSecureMcpLauncher(serverUrl: string, clientKey: string): { command: string; args: string[] } {
  const dir = path.join(getHomeDir(), ".secretvault");
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 });
  const credPath = path.join(dir, "mcp-credential.key");
  fs.writeFileSync(credPath, clientKey, { mode: 0o600 });
  // Best-effort tighten permissions after write (idempotent on POSIX).
  try { if (process.platform !== "win32") fs.chmodSync(credPath, 0o600); } catch { /* best effort */ }

  if (process.platform === "win32") {
    // Windows: a tiny Node launcher the config invokes via `node <launcher>`.
    const launcherPath = path.join(dir, "mcp-launcher.cjs");
    fs.writeFileSync(launcherPath, [
      `#!/usr/bin/env node`,
      `// Auto-generated by secretvault setup. Reads the client key at runtime.`,
      `const { readFileSync } = require("node:fs");`,
      `const { spawn } = require("node:child_process");`,
      `const key = readFileSync(${JSON.stringify(credPath)}, "utf8").trim();`,
      `const child = spawn("npx", ["-y",${JSON.stringify(MCP_REMOTE_SPEC)},${JSON.stringify(`${serverUrl}/mcp`)},"--header",\`Authorization: Bearer \${key}\`,"--allow-http"], { stdio: "inherit", shell: true });`,
  ...
  // POSIX: a shell launcher. The key is read at runtime, never stored in argv.
  const launcherPath = path.join(dir, "mcp-launcher.sh");
  fs.writeFileSync(launcherPath, [
    `#!/usr/bin/env sh`,
    `# Auto-generated by secretvault setup. Reads the client key at runtime;`,
    `# the key is never placed in this script's argument list.`,
    `set -eu`,
    `key="$(cat ${credPath})"`,
    `exec npx -y ${MCP_REMOTE_SPEC} ${serverUrl}/mcp --header "Authorization: Bearer $key" --allow-http`,
    ``,
  ].join("\n"), { mode: 0o700 });
  try { fs.chmodSync(launcherPath, 0o700); } catch { /* best effort */ }
...
export async function handleSetupCli(): Promise<void> {
  ...
  try {
    ...
    // SV-AUD-007: never embed the client key in the generated MCP config's
    // argument list (it persists on disk and shows up in process listings).
    // Write the key to a 0600 credential file and a small launcher script that
    // reads it at runtime and builds the Authorization header, then point the
    // tool config at the launcher. The config args contain no secret material.
    const launcher = writeSecureMcpLauncher(serverUrl, clientKey);
